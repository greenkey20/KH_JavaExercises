...
// 2022.5.19(목) 23h50 -> 2022.5.20(금) 0h35 왜 틀리는지 잘 모르겠음 ㅠ.ㅠ -> 2022.5.20(금) 1h10 232ms 맞았습니다 
			...
			for (int j = 0; j < n; j++) {
				if (i != j) {
					if (list[i][0] < list[j][0] && list[i][1] < list[j][1]) {
						/* 나(i 반복문으로 선택된 사람)의 덩치가 타인(j 반복문으로 선택된 사람)의 덩치보다 같거나 작은 경우(x) 작은 경우에만(o), 나의 등수 커짐
						 * vs 나의 덩치가 타인의 덩치와 같거나, 나-타인의 덩치를 비교할 수 없는 경우, 순위를 매길 수 없음
						 */
						ranks[i] += 1;
					}
				}
