...
public class VariablePractice1 { // 2021.10.15(금) 변수과제1 클래스 영역 시작 
...
//		System.out.printf("사각형의 둘레 : %.2f\n", (width + height) * 2); // 계산 결과의 표시를 소수점 이하 몇 자리로 특정할 때 사용하려면 이 코드 사용
	...
	public void methodQ3() { // 3번 문제 풀이를 위한 메서드 영역 시작
		...
		String str = sc.nextLine(); // 개행문자가 나오기 전까지의 문장/문자열을 입력받는 nextLine() 메서드를 이용해서 사용자로부터 입력받은 문자열을 참조자료형 클래스 String으로 str라는 이름/변수/저장공간에 저장
//		char c1 = sc.nextLine().charAt(0); // 아래 3줄처럼 입력받으면 문자열 3개를 입력받게 되는 바, 이 코드들 사용하지 않음 
...
//		char c3 = sc.nextLine().charAt(2);
	...
	public void methodQ4() { // 4번 문제 풀이를 위한 메서드 영역 시작
		...
		String name = sc.nextLine(); // 개행문자가 나오기 전까지의 문장/문자열을 입력받는 nextLine() 메서드를 이용해서 사용자로부터 입력받은 사용자 이름을 참조자료형 클래스 String으로 name이라는 이름/변수/저장공간에 저장
		...
		double grade = sc.nextDouble(); // 실수값을 입력받는 nextDouble() 메서드를 이용해서 사용자로부터 입력받은 성적(실수값)을 실수형(double) 변수 grade에 저장
