...
public class B_Else { // B_Else 클래스 영역 시작
	...
	public void method1() { // method1 영역 시작
		...
		// A_If class > method1()에서 if문만 쓴 경우: if문 3개 적용/연산 필요함
...
//		}
	...
	public void method6() { // method6 영역 시작; switch 수업시간 중에 했는데, 내가 이 클래스(관련 주제)로 옮겨둠
		// 로그인 기능(같은 것 만들기)
		...
		if (userId.equals(id) && userPwd.equals(pwd)) { // String 참조(자료)형 type에서 사용 가능한 기능을 활용해서 비교
			// ==는 값을 비교하는 동등비교 연산자인데, 참조자료형인 String에는 주소값이 저장되어 있기 때문에, String 변수 이름끼리 == 동등비교하면 각 변수의 문자열이 저장되어 있는 주소값이 비교됨(o) 문자열 비교(x)
		...
		} else if (!userId.equals(id) && userPwd.equals(pwd)) {
