...
public class C_Switch { // 클래스 C_Switch 영역 시작
	...
	public void method1() { // method1 영역 시작
		...
		int oddEven = num % 2; // 가능한 경우의 수는 두 가지(0: 짝수, 1: 홀수)
		
		// oddEven == 0이면 위 -> 아래 모든 스위치/cases 모두 실행 -> 아래 스위치 모두 실행시키지 않으려면 break
		// oddEven == 1이면 (엘리베이터 스위치 버튼처럼) 바로 값 1 case로 점프 
		switch(oddEven) { // switch 우측 괄호 안에 쓴 조건(?)에 따라 case 값 결정됨..
		
		case 0 : System.out.println("짝수입니다"); // oddEven == 0인 경우
			break; // switch문에서 벗어나기/탈출하기 위해서 씀
		case 1 : System.out.println("홀수입니다"); // oddEven == 1인 경우
			break; // 맨 아래 case는 break 필요 없지만, 추후 수정 시를 대비하여 적어주는 게 낫다..
		}
	...
	public void method2() { // method2 영역 시작
		...
		// 문자열 변수를 선언 및 대입 <- 문자열을 입력받는 스캐너 기능(Scanner라는 특정 클래스에서 사용할 수 있는 메서드 중 하나)을 이용
		// 문자열 = 순서가 있는 문자의 집합
		
		// 2. 입력받은 주민번호로부터 성별에 해당하는 문자 추출
		char gender = personId.charAt(7);
		// 단일문자 변수를 선언 및 대입 <- 문자열에서 내가 지정한 index에 있는 단일문자 하나만 추출하는 기능을 이용
		...
		case '1' : // 변수 gender의 값과 동등비교할 '문자' literal -> gender == '1'
	...
	public void fruitMarket() { // fruitMarket 메서드 영역 시작
		...
		System.out.println("-- Welcome, Green Fruit Market --");
		...
		default : System.out.println("죄송합니다, 저희 가게에서 판매하지 않는 과일입니다. 안녕히 가세요!");
					return; // switch > default는 할일 다 마쳤으니까 자신을 부른 곳(Run 클래스의 main 메서드)으로 돌아감
							// 현재 실행되고 있는 메서드 영역을 빠져나가 그 메서드를 호출한 부분으로 돌아감
 		}
...
//		}
	...
	public void method4() { // method4 영역 시작
		...
		// 나의 간이/작은 database(DB)
		String id = "greenkey20";
		String pwd = "pass1234";
		
		if (userId.equals(id) && userPwd.equals(pwd)) { // String 참조형 type에서 사용 가능한 기능을 활용해서 비교
			// ==는 값을 비교하는 동등비교 연산자인데, String에는 주소값이 저장되어 있기 때문에
		...
		// 기본자료형 boolean, char, byte, short, int, long, float, double의 경우 동등비교(==) 가능
		// 참조자료형(String 등)은 동등비교(==) 불가능; 참조자료형의 경우 실제로 변수에 담기는 값이 '주소값'이기 때문
		// -> 문자열 비교 시 equals() 메서드 사용 -> 결과는 true, false
