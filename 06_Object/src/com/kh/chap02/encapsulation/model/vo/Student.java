...
// 클래스/value object = 필드부 + 생성자부 + 메소드부

public class Student {
	
	// [필드부]
	/* 각 정보를 담는 부분
	 * 필드 == (클래스의) 멤버 변수 == 인스턴스변수 == 속성 (== 클래스(의) 변수) cf. chap04 > FieldTest1 참고
	 * 접근제한자 자료형 필드이름;
	 */
	
	// 캡슐화 단계1) 앞으로 클래스 안 필드 선언할 때는 private으로 설정 -> 값 숨기기
	 ...
	 * 매개변수: 메소드 호출 시 입력값으로 넣어주는 변수; 메소드 호출 시에는 '인자/argument'라고 부름; 해당 메소드 실행 중에만 사용 가능한 변수; 생략 가능; 각기 다른 자료형으로 2개 이상의 값 입력 가능
	 */
	
	/* 캡슐화 단계2) 클래스 외부에서 간접적으로 값을 처리할 수 있는 getter(값 가져오기), setter(값 대입) 메소드 만듦
	 * 
	...
	public void setName(String name) { // 반환할 것이 없으므로 void; Student처럼 생긴 객체의 이름을 대입/set하고자 함; 문자열인 name을 매개변수로 입력함; 군청색 = 블록{} 안에서만 쓰이는 지역변수
		this.name = name; // 메소드의 매개변수로 입력한 값(여기서는 name)을 이(this) 객체 필드에 대입함; this(주황색 = 키워드; '객체'를 의미).가 없으면 name은 모두 중괄호{} 영역 안의 지역변수로 인식됨
	}
	...
	public void setGender(char gender) {
		this.gender = gender;
		// 특정 영역 안에서는 해당 영역 안에 있는 지역변수가 우선권이 있음 -> 여기서는 매개변수 gender가 우선권이 있음; this.를 붙이지 않으면 매개변수 gender로 인식이 됨
	}
	...
	public String getName() { // 매개변수/입력은 필요 없음 
