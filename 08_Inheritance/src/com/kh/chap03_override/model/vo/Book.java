...
public class Book /* extends Object */ { // 모든 클래스의 최상위 부모클래스 = Object -> 생략 가능
	 ...
	 * 3. 반환형이 동일
	 * 4. 부모메소드의 접근제한자보다 같거나 공유 범위가 더 넓어야 함 -> 접근제한자를 바꾸는 경우는 많지 않으니 이 점을 신경쓸 일은 별로 없을 것임; '규약' 개념
	 * 
	 * @Override
	 * annotation; 일종의 주석; 생략 가능하지만, 작성하는 습관 들이기 -> 실수 줄일 수 있도록
	 *  cf. codes에서 주석 = 프로그램 실행 시 실행 안 됨/무시됨
	 * 명시 안 해도 부모메소드와 형태가 같으면 overriding된 것으로 판단
	 * annotation 붙이는 이유: 누가 봐도 overriding이라고 표시(?) -> 실수를 줄이는 방법이라서 -> 생산성 향상
	 */
