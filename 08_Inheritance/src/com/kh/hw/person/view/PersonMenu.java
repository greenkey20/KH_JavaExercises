package src.com.kh.hw.person.view;
...
public class PersonMenu {
	...
	public void studentMenu() { // 학생 메뉴를 출력하는 메소드
		while (true) {
			...
			if (count == pc.printStudent().length) {
			...
			switch (menu) {
			case 1 : if (count == pc.printStudent().length) {
...
//				mainMenu();
	...
	public void employeeMenu() { // 사원 메뉴를 출력하는 메소드
		while (true) {
			...
			if (count == pc.printEmployee().length) {
			...
			switch(menu) {
			case 1 : if (count == pc.printEmployee().length) {
						System.out.println("잘못 입력하셨습니다. 다시 입력해주세요~");
					} else {
						insertEmployee();
					}
