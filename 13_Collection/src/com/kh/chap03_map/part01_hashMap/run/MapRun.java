// 2021.11.19(금) 12h
package src.com.kh.chap03_map.part01_hashMap.run;

import src.com.kh.chap03_map.part01_hashMap.model.vo.Burger;
...
public class MapRun {
    ...
    public static void main(String[] args) {
        ...
        // 1. put(K key, V value) -> 맵 공간에 key + value 세트 추가
        ...
        // 기존의 key값으로 다른 value를 추가하면/key값이 중복되면 덮어씌워짐 -> 나중에 그 key를 사용한 요소의 값으로 value가 변경됨
...
//		Iterator itKey = keySet.iterator();
        ...
        System.out.println("===HashMap에 들어있는 모든 요소들 조회하는 방법1) keySet() 이용===");
...
//			Object key = itKey.next(); // Iterator에 제네릭 제시하지 않은 경우
        ...
        // 단계1) Map에 있는 key + value 세트를 Entry형식으로 Set에 담기
        Set<Entry<String, Burger>> entrySet = hm.entrySet();
        // 제네릭 사용 이유 = 컬렉션에 들어가는 자료형을 제한 -> Set의 요소로 Entry자료형만 들어갈 수 있음 -> EntrySet에는 String타입 key와 Burger타입 value가 Entry라는 형식으로 묶여서; Entry처럼 생긴 String타입 key와 Burger타입 value가 들어감
        ...
        // 단계3) 반복문 이용
        System.out.println("===HashMap에 들어있는 모든 요소들 조회하는 방법2-a) entrySet() 이용 + while문===");
        while (itEntry.hasNext()) {
            ...
            // 16h while문 내용 마저 필기 못함 -> 2023.9.10(일) 0h30 내가 알고 있는 대로 작성해봄
            String key = e.getKey();
            Burger value = e.getValue();

            System.out.println("key : " + key + " = value " + value);
        }

        // for문도 필기 못함 -> 수업 화면 캡처 내역 참고해서 2023.9.10(일) 0h35 내가 아는 대로 작성해봄
        System.out.println("===HashMap에 들어있는 모든 요소들 조회하는 방법2-b) entrySet() 이용 + enhanced for문===");
